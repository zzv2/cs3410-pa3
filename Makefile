...
# optimizations, especially wherever you use inline assembly. The "lordofthecache.h"
...
LINKFLAGS = $(FLAGS) -nostartfiles -nodefaultlibs -Wl,-T,src/lordofthecache.x
...
$(TARGETS): bin/%: obj/%.o src/lordofthecache.x Makefile src/lordofthecache.h
	$(CC) $(LINKFLAGS) $< -o $@

# 'make <name>.o' just compiles and assembles one file, but does not link
$(OBJECTS): obj/%.o: src/%.c src/lordofthecache.h Makefile
	$(CC) $(FLAGS) -c $< -o $@

# 'make <name>.s' just compiles that file, but does not assemble or link
$(ASSEMBLIES): src/%.s: src/%.c src/lordofthecache.h Makefile
	$(CC) $(FLAGS) -S $< -o $@
