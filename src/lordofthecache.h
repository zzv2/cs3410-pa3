 ...
 *
 * The upper half of memory (above 0x80000000) is accessible only when in Equip_Ring mode.
 *
 ...
 * data segment.  But if player 1 is in Equip_Ring mode  and tries to dereference 0x00100016,
...
/* How many Balrogs and Nazgul Screeches each team has */
#define BALROGS_PER_TEAM 15
#define NAZGUL_SCREECHES_PER_TEAM 30

/* The bytes representing Balrog, Nazgul Screech, and Use Gandalf */
#define BALROG 0xF0
#define NAZGUL_SCREECH  0xF1
#define USE_GANDALF 0xF2

/* Number of cycles a core will stall in the following situations */
#define STALL_TO_SUMMON_BALROG 100
#define WAIT_FOR_GANDALF 100
#define FLY_YOU_FOOLS 1000000
#define STALL_WHEN_STUNNED 300
#define STALL_ON_DISTRACTED 100000

/* After a Nazgul Screech is used, opponents will be hit if they write within
 * the next NAZGUL_SCREECH_DURATION cycles
 */
#define NAZGUL_SCREECH_DURATION 100
 ...
 * your opponent's status when you are in Equip_Ring mode.  This datastructure is exactly 256 bytes
...
struct player_status {
   ...
   * num_balrogs: Number of Balrogs this team has used already.
   * num_nazgul_screeches: Number of nazgul_screeches this team has used already.
   * The padding is unused, but ensures the datastructure will exactly
   * fill 256 bytes of memory.
   */
  unsigned char payload, num_balrogs, num_nazgul_screeches, pad0;
  /* Each element of taunt[] will contain a core id between 0 and 3, or -1 to mean "empty".
   * Each player's taunt[] array contains evidence that their opponent is in Equip_Ring mode.
...
// go into Equip_Ring mode so that you can read or write high memory
__attribute__ ((unused)) static void Equip_Ring(void);

// accuse opponent core of being in Equip_Ring mode; returns 1 for success, 0 for false accusation
// if the opponent's core is in Equip_Ring mode, it will be given the carbon frozen (halted forever);
// otherwise, this core will be penalized by stalling some number of cycles
#define FALSE_ACCUSATION_PENALTY 100000
__attribute__ ((unused)) static int Eye_Of_Sauron(int opponent_core_id);

// Unequip_Ring from Equip_Ring mode
__attribute__ ((unused)) static void Unequip_Ring(void);
...
__attribute__ ((unused)) static int Eye_Of_Sauron(int id) {
